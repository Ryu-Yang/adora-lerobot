...
from lerobot.common.robot_devices.robots.configs import AdoraRobotConfig
...
class AdoraManipulator:
    ...
    #     **kwargs,
    # ):
    def __init__(self, config: AdoraRobotConfig):
        self.config = config
        self.robot_type = self.config.type
        self.calibration_path = Path(self.config.calibration_dir)
        self.leader_arms = make_motors_buses_from_configs(self.config.leader_arms)
        self.cameras = make_cameras_from_configs(self.config.cameras)
        self.is_connected = False
        self.logs = {}
        self.start_pose = config.start_pose
        self.joint_p_limit = config.joint_p_limit
        self.joint_n_limit = config.joint_n_limit
        #  gen72机械臂接入，dll文件路径
        dllPath = DLL_PATH
        self.pDll = ctypes.cdll.LoadLibrary(dllPath)
        #  连接机械臂
        self.pDll.RM_API_Init(72,0)
        byteIP = bytes(config.ip,"gbk")
